...
class Case:
    ...
    def __post_init__(self):
        species, reactions, inerts = tuple(self.species), tuple(self.reactions), tuple(self.inert_species)
        ids = tuple(item.id for item in species)
        if not self.name: raise ValueError("Case name must not be empty.")
        if len(ids) != len(set(ids)): raise ValueError("Case species must not contain duplicates.")
        if ids != self.symbols.species_ids: raise ValueError("Case species and symbols do not match.")
        if self.domain_spec.symbols != self.symbols: raise ValueError("Case and domain symbols differ.")
        reaction_ids = tuple(item.id for item in reactions)
        if not reactions: raise ValueError("Case must contain at least one reaction.")
        if len(reaction_ids) != len(set(reaction_ids)): raise ValueError("Reaction ids must be unique.")
        ...
        for reaction in reactions:
            ...
            if missing: raise ValueError(f"Reaction '{reaction.id}' requires missing species: " +
                                         ", ".join(sorted(missing)) + ".")
            ...
            if unknown: raise ValueError(f"Reaction '{reaction.id}' uses symbols outside this case: " +
        ...
        for key, value in (("species", species), ("reactions", reactions),
                           ("inert_species", inerts), ("check_config", dict(self.check_config)),
