...
from ..proof import derive_network_lipschitz
...
def _rate_finding(reaction_id, result):
    ...
    if shown:
        summary += f" Decisive expression: {shown}."
    data = {}
    ...
    return Finding(
        reaction_id, verdict, summary, Evidence("lipschitz_obstruction", data) if data else None
    )


def _network(context, domain, certificates):
    certificate = derive_network_lipschitz(
        domain, context.case.symbols.species_ids, context.stoichiometry, certificates
    )
    return Finding(
        context.case.name,
        Verdict.PASS,
