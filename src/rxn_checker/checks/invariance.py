    ...
    for species_id in context.case.symbols.species_ids:
        ...
        for reaction in context.case.reactions:
            ...
            if coefficient is None:
                continue
            contribution = {"reaction": reaction.id, "stoichiometric_coefficient": coefficient}
            (consumers if coefficient < 0 else producers).append(contribution)
        faces[species_id] = {
            "vanishing_consumers": tuple(consumers),
...
def run_forward_invariance(
    context: AnalysisContext, dependencies: Mapping[str, CheckResult]
) -> Finding:
    boundary = dependencies["physical_boundary_inward"].findings[0].evidence
    regularity = next(
        finding.evidence
        for finding in dependencies["physical_lipschitz"].findings
        if finding.evidence and finding.evidence.kind == "network_lipschitz_certificate"
    )
    parameters = {
        str(symbol): {"lower": interval.lower, "upper": interval.upper}
