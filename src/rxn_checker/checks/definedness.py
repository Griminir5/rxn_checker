...
def _finding(reaction_id, result, domain):
    ...
    if verdict is Verdict.PASS:
        return Finding(reaction_id, verdict, f"Rate is real and finite on the {domain} domain.")
    decisive = result.decisive_subexpression
    summary = (
        f"Rate is not defined throughout the {domain} domain."
    ...
    if result.witness:
        data["point"] = {str(key): str(value) for key, value in result.witness.items()}
    return Finding(
        reaction_id, verdict, summary, Evidence("definedness_guard", data) if data else None
    )
...
def run_physical(context, _dependencies):
    return _run(context, context.physical_domain)


def run_augmented(context, _dependencies):
