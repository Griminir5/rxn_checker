...
from ..proof import ProofVerdict, SignRequirement, SumProof
from ..results import CheckResult, Evidence, Finding, Verdict


def _data(result, reaction_ids) -> dict[str, object]:
    data = {
    ...
    if result.witness is not None:
        data.update(
            point={str(symbol): value for symbol, value in result.witness.items()},
...
def _label(result) -> tuple[str, bool]:
    if result.reason == "No feasible negative points.":
        return "not applicable", False
    label = {
        ProofVerdict.PASS: "proved",
        ProofVerdict.FAIL: "disproved",
...
def _finding(species_id, symbol, non_repulsion, attraction, reaction_ids):
    verdict = Verdict(non_repulsion.verdict)
    main, _ = _label(non_repulsion)
    strict, stuck = _label(attraction)
    summary = (
        f"Non-repelling for {symbol} <= 0: {main}; strictly attracting for {symbol} < 0: {strict}."
    )
    if stuck:
...
def run(context: AnalysisContext, dependencies: Mapping[str, CheckResult]) -> tuple[Finding, ...]:
    ...
    regular = {item.subject: item.verdict for item in dependencies["augmented_lipschitz"].findings}
    findings = []
    for species_id in context.case.symbols.species_ids:
        ...
        missing = tuple(
            reaction.id for reaction in reactions if regular.get(reaction.id) is not Verdict.PASS
        )
        if missing:
            findings.append(
                Finding(
        ...
        attraction = (
            context.expression_analyzer.prove_sum(terms, negative, SignRequirement.POSITIVE)
            if negative.is_feasible()
