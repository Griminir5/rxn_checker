...
XU_FROMENT_ACTIVATION_ENERGIES_J_PER_MOL = {"smr": 240100.0, "wgs": 67130.0, "overall": 243900.0}
XU_FROMENT_ADSORPTION_COEFFICIENTS = {"CO": 8.23e-10, "H2": 6.12e-14, "CH4": 6.65e-9, "H2O": 1.77e5}
...
def _equilibrium(kind, temperature):
    ...
    return sp.exp(
        sum(coefficient * scaled**power for power, coefficient in enumerate(_EQUILIBRIUM[kind]))
    )
...
def xu_froment_terms(symbols: CaseSymbols) -> XuFromentTerms:
    ...
    p_ch4, p_co, p_co2, p_h2, p_h2o = (
        pressure * symbols.concentration(species) / total
        for species in ("CH4", "CO", "CO2", "H2", "H2O")
    )
    ...
    denominator = (
        1
        + _adsorption("CO", temperature) * p_co
...
def _common(terms, kind, inverse_exponent, pressure_scale):
    rate = (
        ...
        * XU_FROMENT_RATE_COEFFICIENTS[kind]
        * sp.exp(
            -XU_FROMENT_ACTIVATION_ENERGIES_J_PER_MOL[kind]
            / (GAS_CONSTANT_J_PER_MOL_K * terms.temperature_k)
        )
    )
    return rate * terms.p_inv_h2_pa_inv**inverse_exponent / (pressure_scale * terms.denominator**2)


def _rate(terms, kind, backward=False):
    if kind == "smr":
        drive = (
            terms.p_h2_pa**3 * terms.p_co_pa / (1e10 * _equilibrium(kind, terms.temperature_k))
            if backward
            else terms.p_ch4_pa * terms.p_h2o_pa
        )
        return _common(terms, kind, sp.Rational(5, 2), 10.0**-2.5) * drive
    if kind == "wgs":
        drive = (
            terms.p_h2_pa * terms.p_co2_pa / _equilibrium(kind, terms.temperature_k)
            if backward
            else terms.p_co_pa * terms.p_h2o_pa
        )
        return _common(terms, kind, sp.S.One, 1.0e5) * drive
    drive = (
        terms.p_h2_pa**4 * terms.p_co2_pa / (1e10 * _equilibrium(kind, terms.temperature_k))
        if backward
        else terms.p_ch4_pa * terms.p_h2o_pa**2
    )
...
def build_family(symbols: CaseSymbols) -> Mapping[str, Reaction]:
    ...
    specs = (
        ("wgs", {"CO": 1, "H2O": 1}, {"CO2": 1, "H2": 1}),
        ("smr", {"CH4": 1, "H2O": 1}, {"CO": 1, "H2": 3}),
        ("overall", {"CH4": 1, "H2O": 2}, {"CO2": 1, "H2": 4}),
    )
    family = {}
    for suffix, backward in (("fw", False), ("bw", True)):
        for kind, reactants, products in specs:
            name = f"{kind}_{suffix}"
            if backward:
                reactants, products = products, reactants
            family[name] = Reaction(
                f"xu_froment.{name}", reactants, products, ("Ni",), _rate(terms, kind, backward)
            )
    return family
